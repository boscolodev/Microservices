...
docker run -p 5432:5432 --name hr-user-pg12 --network hr-net -e POSTGRES_PASSWORD=123456 -e POSTGRES_DB=db_hr_user postgres:12-alpine
```
...
docker run -p 8888:8888 --name hr-config-server --network hr-net -e github_username=boscolodev -e github_password= hr-config-server:v1
```
